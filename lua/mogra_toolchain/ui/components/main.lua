...
return function(state)
  ...
  for _, tool in ipairs(tools) do
    ...
    elseif install_state == "failed" then
  ...
  local any_installing_or_checking = #installing > 0 or state.tools.checking_statuses or _.any(function(tool)
    return tool.install_state == "checking"
  end, tools)
  ...
  local function render_tool(tool, show_log)
    ...
    -- Show log output for installing or failed tools
    if show_log and (is_installing or is_failed) and tool.tailed_output and tool.tailed_output ~= "" then
      local log_lines = vim.split(tool.tailed_output, "\n")
      -- Show more lines when expanded, fewer when collapsed
      local max_lines = tool.is_log_expanded and 15 or 2
      local total_lines = #log_lines
      local start_line = math.max(1, total_lines - max_lines + 1)
      ...
      -- Show expand/collapse hint if there are more lines
      if total_lines > max_lines then
        current_line = current_line + 1
        local hint_text = tool.is_log_expanded and "  ▲ Press 'l' to collapse (" .. total_lines .. " lines)" or "  ▼ Press 'l' to expand (" .. (total_lines - max_lines) .. " more lines)"
        table.insert(
          items,
          Ui.HlTextNode({
            { { hint_text, "MograToolchainComment" } },
          })
        )
      end
    end
  ...
  if #failed > 0 then
    add_line(
      Ui.HlTextNode({
        { { "Failed", "MograToolchainError" }, { " (" .. #failed .. ")", "MograToolchainMuted" } },
      }),
      nil
    )
    for _, tool in ipairs(failed) do
      render_tool(tool, true) -- show log for failed
    end
    add_empty_line()
  end
  ...
  -- Footer
  local footer_text = " i: Install  u: Update  l: Toggle log  q: Quit"
