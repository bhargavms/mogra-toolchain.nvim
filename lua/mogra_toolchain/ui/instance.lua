...
require("mogra_toolchain.ui.colors").setup()
...
local function GlobalKeybinds(_state)
  return Ui.Node({
    ...
    Ui.Keybind("l", "TOGGLE_LOG", nil, true),
...
local function setup_tools()
  mutate_state(function(s)
    ...
    s.tools.all = _.map(ToolState.new, settings.current.tools)
  end)
...
local function start_refresh_timer()
  ...
  refresh_timer:start(100, 100, function() -- Changed to 100ms for smoother spinner animation
    vim.schedule(function()
      ...
      local current_state = get_state()
      local current_count = #current_state.tools.all
      local settings_count = #settings.current.tools

      -- Fast-fail: length check first
      if current_count ~= settings_count then
        setup_tools()
      elseif current_count > 0 then
        -- Same length: compare tool identities to detect swaps
        local tools_changed = false
        for i = 1, current_count do
          local current_name = current_state.tools.all[i] and current_state.tools.all[i].name
          local settings_name = settings.current.tools[i] and settings.current.tools[i].name
          if current_name ~= settings_name then
            tools_changed = true
            break
          end
        end
...
local function toggle_log(_event)
  mutate_state(function(s)
    for i, _ in ipairs(s.tools.all) do
      s.tools.all[i].is_log_expanded = not s.tools.all[i].is_log_expanded
    end
  end)
end
